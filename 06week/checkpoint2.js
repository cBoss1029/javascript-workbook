...
const sumEvens = nums.filter((num) => num % 2 === 0).reduce((total, current) => total + current);
...
nums.forEach((item) => {
  numCount[item] = numCount[item] + 1;
});
// This works if you don't want to type out all the keys for numCount first.


// const numCount = {};
// nums.forEach((item)=>{
//   if (isNaN(numCount[item])) {
//     numCount[item] = 1;
//   } else {
//     numCount[item] = numCount[item] + 1;
//   }
